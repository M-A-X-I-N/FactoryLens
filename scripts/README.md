...
- `FL-B150-validate-external-overrides.ps1` — resolves the supported JDK 25 + clangd 20 environment, regenerates/audits the UBT Clang compile view, then runs the production foreground-AST external-override provider across RSS2 headers with streamed progress and checks representative positive/negative B6 controls.
